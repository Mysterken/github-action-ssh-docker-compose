...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...' ; } ; . ~/.bash_profile ; log 'Recreating workspace directory...' ; sudo rm -rf \"\$HOME/workspace\" ; mkdir -p \"\$HOME/workspace\" ; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/workspace\" -xjv ;log 'Logging in the container registry $CONTAINER_REGISTRY'; log 'Launching docker compose...' ; cd \"\$HOME/workspace\"  ; make prod"
